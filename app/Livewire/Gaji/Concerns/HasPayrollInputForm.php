...
trait HasPayrollInputForm
{
    ...
    public function generateBulkDraftSlips(PayrollService $payrollService): void
    {
        try {
            $count = $payrollService->generateBulkDraftSlips($this->periode);
            $this->toast()->success('Berhasil !', "Data gaji untuk {$count} karyawan berhasil disalin/dibuat. Status input kini 'Selesai'.")->send();
        } catch (\Throwable $e) {
