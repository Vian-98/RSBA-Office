<form wire:submit.prevent="submit" class="flex flex-col gap-4">
    @if($suratBalasanPenelitian)
        <div class="rounded-lg bg-slate-50 p-3 border border-slate-200 text-xs">
            <span class="font-bold text-slate-700 block">{{ $suratBalasanPenelitian->no }}</span>
            <span class="text-slate-500 block">{{ $suratBalasanPenelitian->tujuan_fakultas }} — {{ $suratBalasanPenelitian->tujuan_universitas }}</span>
            <span class="text-slate-600 font-medium block mt-1">Total: Rp {{ number_format($suratBalasanPenelitian->total_biaya, 0, ',', '.') }} ({{ $suratBalasanPenelitian->mahasiswa->count() }} Peneliti)</span>
        </div>
        ...
        <div class="ml-auto flex items-center gap-2 pt-2">
            <x-ts:button type="button" outline color="secondary" sm x-on:click="$dispatch('close-modal', {id: 'modal-approval-balasan-penelitian'})">
                Batal
            </x-ts:button>
