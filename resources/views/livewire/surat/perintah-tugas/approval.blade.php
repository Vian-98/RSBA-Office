<form wire:submit.prevent="submit" class="flex flex-col gap-4">
    @if($suratPerintahTugas)
        <div class="rounded-lg bg-slate-50 p-3 border border-slate-200 text-xs">
            <span class="font-bold text-slate-700 block">{{ $suratPerintahTugas->no }}</span>
            <span class="text-slate-600 block mt-0.5">{{ $suratPerintahTugas->perihal }}</span>
            <span class="text-slate-500 block mt-1">{{ $suratPerintahTugas->hari_tanggal }} • {{ $suratPerintahTugas->tempat }}</span>
        </div>
        ...
        <div class="ml-auto flex items-center gap-2 pt-2">
            <x-ts:button type="button" outline color="secondary" sm x-on:click="$dispatch('close-modal', {id: 'modal-approval-perintah-tugas'})">
                Batal
            </x-ts:button>
