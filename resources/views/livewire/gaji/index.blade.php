<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-2xs">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
            ...
            <div>
                <x-ts:input label="" wire:model.live="periode" type="month" class="w-full" />
            </div>
        </div>
    </div>
            ...
            <div class="p-2 space-y-5">
                ...
                <form wire:submit.prevent="savePayroll" class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block pb-1 border-b border-slate-100">Komponen Pendapatan (+)</span>
                            
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <x-ts:input label="Gaji Pokok (Base)" wire:model.defer="form_gaji_pokok" type="number" prefix="Rp" disabled class="bg-slate-100 cursor-not-allowed font-semibold text-slate-700" />
                                <x-ts:input label="Tunjangan Tetap" wire:model.defer="form_tunjangan_tetap" type="number" prefix="Rp" disabled class="bg-slate-100 cursor-not-allowed font-semibold text-slate-700" />
                                <x-ts:input label="Tunjangan Absensi" wire:model.defer="form_tunjangan_absensi" type="number" prefix="Rp" disabled class="bg-slate-100 cursor-not-allowed font-semibold text-slate-700" />
                                <x-ts:input label="Tunjangan Jabatan" wire:model.defer="form_tunjangan_jabatan" type="number" prefix="Rp" disabled class="bg-slate-100 cursor-not-allowed font-semibold text-slate-700" />
                                ...
                                <div class="col-span-1 sm:col-span-2 bg-slate-50 border border-slate-100 rounded-2xl p-4 mt-2 text-xs font-semibold text-slate-600">
                                    <span class="text-2xs text-indigo-600 font-bold uppercase tracking-wider block mb-2">Rincian Alokasi UMK (25% UMK)</span>
                                    @foreach($form_umk_allocations as $alloc)
                                        <div class="flex justify-between py-0.5">
                                            <span>{{ $alloc['nama'] }} ({{ $alloc['persen'] }}%)</span>
                                            <span class="text-slate-800">Rp {{ number_format($alloc['nominal'], 0, ',', '.') }}</span>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="col-span-1 sm:col-span-2 border-t border-slate-100 pt-4 mt-2">
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-3 items-end mb-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        <div class="flex-1 w-full">
                                            <label class="block text-xs font-semibold text-slate-500 mb-1">Pilih Jenis Tunjangan</label>
                                        ...
                                        <div class="w-full sm:w-40">
                                            <x-ts:input label="Nominal" wire:model.defer="temp_allowance_nominal" type="number" prefix="Rp" />
                                        </div>
                                        <div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <x-ts:input label="Potongan Bank" wire:model.live.debounce.500ms="form_potongan_bank" type="number" prefix="Rp" />
                                <x-ts:input label="Anggota Keluarga Add-on BPJS" wire:model.live.debounce.500ms="form_bpjs_keluarga_tambahan" type="number" min="0" hint="+1% per kepala" />
                            </div>

                            <!-- Automatic Deductions Preview -->
                            <div class="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-2 mt-4 text-xs font-semibold text-slate-600">
                                <span class="text-2xs text-slate-400 uppercase tracking-wider block mb-1">Potongan Otomatis (Auto)</span>
                                <div class="flex justify-between">
                                    <span>Pot. BPJS Kesehatan (1% + Add-on)</span>
                                    <span class="text-slate-800">Rp {{ number_format($calc_bpjs_kes, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Pot. BPJS Ketenagakerjaan (3%)</span>
                                    <span class="text-slate-800">Rp {{ number_format($calc_bpjs_tk, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>PPh Pasal 21 (Pajak 5%)</span>
                                    <span class="text-slate-800">Rp {{ number_format($calc_pph21, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
