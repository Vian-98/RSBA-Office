<div class="space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 xl:flex-row xl:items-center bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-2xs">
        ...
        <div class="flex flex-wrap items-center justify-start xl:justify-end gap-2 shrink-0">
            ...
            <x-ts:button wire:click="downloadTemplate" flat color="emerald" class="text-xs font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 whitespace-nowrap shrink-0">
                <x-tabler-file-download class="h-4 w-4 mr-1.5 shrink-0" />
                Templat Excel
            </x-ts:button>
            @if(!$isLocked)
                <x-ts:button wire:click="openImportModal" flat color="sky" class="text-xs font-bold bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 whitespace-nowrap shrink-0">
                    <x-tabler-file-upload class="h-4 w-4 mr-1.5 shrink-0" />
                    Impor Excel
                </x-ts:button>
            @endif
            <x-ts:button wire:click="exportToExcel" flat color="emerald" class="text-xs font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 whitespace-nowrap shrink-0">
                <x-tabler-file-spreadsheet class="h-4 w-4 mr-1.5 shrink-0" />
                Export Rekap
            </x-ts:button>
            <x-ts:button wire:click="openPeriodLogModal" flat color="indigo" class="text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 whitespace-nowrap shrink-0">
                <x-tabler-history class="h-4 w-4 mr-1.5 shrink-0" />
                Log Edit
            </x-ts:button>
            <x-ts:button href="{{ route('kepegawaian.gaji.index') }}" flat color="slate" class="text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 whitespace-nowrap shrink-0">
    ...
    <x-ts:modal wire="isBatchSendModalOpen" title="Kirim Massal Slip Gaji (Background Queue)" size="lg" class="relative z-50">
        <div class="space-y-5" @if($isBatchSending) wire:poll.1s="refreshBatchProgress" @endif>
            ...
            @php
                $modalFailedLogs = \App\Models\Sdm\PayrollSendLog::with('karyawan')
                ...
                <div class="p-4 bg-rose-50/80 border border-rose-200 rounded-2xl space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2 text-rose-900 font-bold text-xs">
                            ...
                            <span>Detail Email Slip Gaji Gagal Terkirim ({{ $modalFailedLogs->count() }} Karyawan)</span>
                        </div>
                    </div>

                    <div class="max-h-56 overflow-y-auto space-y-2 pr-1">
                        @foreach($modalFailedLogs as $failedLog)
