        ...
        <x-slot:header>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">File: {{ $log->nama_file }} | Periode: {{ \Carbon\Carbon::parse($log->periode_awal)->format('d M Y') }} - {{ \Carbon\Carbon::parse($log->periode_akhir)->format('d M Y') }}</p>
                </div>
                <div>
                    <x-ts:button color="primary" wire:click="commitKeJadwal">Commit ke Jadwal</x-ts:button>
                </div>
            </div>
        </x-slot:header>
